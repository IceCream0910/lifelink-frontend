    ...
    const sendToLLM = async (transcript: string) => {
        try {
            ...
            const model = genAI.getGenerativeModel({
                ...
                systemInstruction: "환자 브리핑 음성 발화가 주어지면, 이를 바탕으로 JSON 데이터를 작성해서 반환해.\n\npatient_age가 구체적으로 명시되지 않은 경우, 대략적인 나이대를 추정해서 작성해(예를 들어 20대인경우 20으로 반환).\npatient_gender는 \"남성\", \"여성\" \"미상\" 중 하나여야 해.\npatient_citizenship는 \"내국인\", \"외국인\", \"미상\" 중 하나여야 해. 별도 언급이 없는 경우 \"내국인\"으로 반환해.\npatient_name이 명시되어 있지 않은 경우 \"미상\"으로 반환해.\npatient_symptom_summary는 브리핑 내용을 바탕으로 최대한 간결하면서도 의학적으로 서술해.\nseverity는 1~5의 정수여야 해. severity의 각 단계 별 설명은 다음과 같아. 1단계(즉각적인 처치가 필요하며 생명이나 사지를 위협하는 또는 악화 가능성이 높은 상태, 예: 심장마비, 무호흡, 음주와 관련되지 않은 무의식), 2단계(생명 혹은 사지, 신체기능에 잠재적인 위협이 있으며 이에 대한 빠른 치료가 필요한 경우, 예: 심근경색, 뇌출혈, 뇌경색), 3단계(치료가 필요한 상태로 진행할 수도 있는 잠재적 가능성을 고려해야 하는 경우, 예: 호흡곤란, 출혈을 동반한 설사), 4단계(환자의 나이, 통증이나 악화/합병증에 대한 가능성을 고려할 때 1-2시간 안에 처치나 재평가를 시행하면 되는 상태, 예: 38도 이상의 발열을 동반한 장염, 복통을 동반한 요로감염), 5단계(긴급하지만 응급은 아닌 상태, 만성적인 문제로 인한 것이거나 악화의 가능성이 낮은 상태, 예: 감기, 장염, 설사, 열상)"
            });
